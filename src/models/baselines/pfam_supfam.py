...
class PfamSUPFAM(BaseModel):
    ...
    def predict_proba(
        ...
        assert isinstance(val_df, pd.DataFrame), "This model does not support class selection."
        assert (
                selected_class_name is None
        ), "This model does not support class selection."
        fasta_str = get_fasta_seqs(val_df[self.config.seq_col_name].values, val_df[self.config.id_col_name].values)
        ...
        for model_path in self.paths_to_models:
            ...
            subprocess.run(f"hmmsearch --noali --notextw --tblout {output_path} -E {self.e_threshold} --cpu {self.config.n_jobs} {model_path} {temp_fasta_path}".split())
            with open(output_path, 'r') as file:
                lines = file.readlines()
            for line in lines:
                if line[0] != '#':
                    entries = line.split()
                    id_2_min_eval[entries[0]] = max(float(entries[5]), id_2_min_eval.get(entries[0], -1000000))
        val_df['isTPS'] = val_df['Uniprot ID'].map(lambda x: id_2_min_eval.get(x, -1000000))

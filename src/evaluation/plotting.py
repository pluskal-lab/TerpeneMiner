...
import numpy as np
import pandas as pd
from seaborn import boxplot, barplot
...
def plot_avg_pr_curves_per_class(
    ...
    colors = plt.get_cmap("tab20" if len(supported_models) > 10 else "tab10")(np.linspace(0, 1, len(supported_models)))
    for model_i, model in enumerate(supported_models):
        ...
        for class_2_vals_fold in class_2_vals:
            for class_, vals in class_2_vals_fold.items():
                if class_ in class_names:
                    precisions_fold, recalls_fold = [list(val[::-1]) for val in vals[:2]]
                    precisions_fold = precisions_fold[:2] + precisions_fold[1:]
                    recalls_fold = recalls_fold[:1] + recalls_fold
                    precision_base = np.interp(base_recall, recalls_fold, precisions_fold)[::-1]
        ...
        print('model: ', model, '; precisions: ', precisions, '; recalls: ', base_recall)
        ...
        ax.plot(
            recall_plot, mean_precisions, color=colors[model_i], label=model_names[model_i]
        )
        ax.fill_between(
            base_recall[::-1],
            precisions_lower,
...
def plot_boxplots_per_type(models: list[str], substr_2_type: dict, model_2_class_2_metric_vals: dict,
                           metric_name: str, title: str, output_path: str, model_names_plots: list[str]):
    model_list = []
    class_list = []
    val_list = []
        ...
        for fold_dict in class_dicts:
            for class_name, val in fold_dict.items():
                if class_name in substr_2_type:
                    ...
                    val_list.append(val)
    ap_per_class_df = pd.DataFrame({'Model': model_list, 'TPS Type': class_list, metric_name: val_list})
    _, ax = plt.subplots(figsize=(15, 8))
    boxplot(x='TPS Type', y=metric_name, hue="Model", data=ap_per_class_df, width=0.4, ax=ax,
            order=[type_name for type_name in ['mono', 'sesq', 'di', 'sester', 'tri', 'tetra']
...
def plot_barplots_per_categories(models: list[str], model_2_class_2_metric_vals: dict, categories_order: list[str],
                                 category_name: str, metric_name: str, title: str, output_path: str, model_names_plots: list[str]):
    model_list = []
    category_list = []
    val_list = []
    ...
    for model_i, model_name in enumerate(models):
        ...
        for fold_dict in class_dicts:
            for category_class_name, val in fold_dict.items():
                assert '_|_' in category_class_name, "The evaluation results provided were not computed per different categories"
                category, class_name = category_class_name.split('_|_')
                present_categories.add(category)
                model_list.append(model_names_plots[model_i])
                category_list.append(category)
                val_list.append(val)
    metric_per_category_df = pd.DataFrame({'Model': model_list, category_name: category_list, metric_name: val_list})
    _, ax = plt.subplots(figsize=(15, 8))
    barplot(x=category_list, y=metric_name, hue="Model", data=metric_per_category_df, width=0.4, ax=ax,
            order=[category_name for category_name in categories_order
...
def plot_selected_results(args: argparse.Namespace):
    ...
    if args.plot_tps_detection:
        detection_specification = '' if args.type_detected == 'isTPS' else args.type_detected
        per_class_results_df = pd.read_csv(eval_output_path / f"per_class_{args.eval_output_filename}.csv").set_index("Model")
        for metric, column_group in zip(['Average Precision', 'ROC-AUC', 'MCC-F1 summary'],
            ...
            if args.type_detected == 'isTPS':
                ...
                series_of_categories_to_check = per_class_results_df['Class'].map(substrate_2_tps_type)
            ...
            plot_bars(means, stds, args.model_names, f"{detection_specification}TPS detection", metric,
                      eval_output_path / f"{plots_name}_{metric}_{detection_specification}TPS.png")

        with open(eval_output_path / f"model_2_class_2_pr_vals{args.eval_output_filename}.pkl", "rb") as file:
            model_2_class_2_pr_vals = pickle.load(file)

        if args.type_detected != 'isTPS':
            classes_to_consider = [class_name for class_name, type_ in substrate_2_tps_type.items() if type_ == args.type_detected]
        else:
            classes_to_consider = ['isTPS']
        plot_avg_pr_curves_per_class(model_2_class_2_pr_vals,
                                     classes_to_consider,
                                     f"{detection_specification}TPS detection Precision-Recall curves",
                                     output_path=eval_output_path / f"{plots_name}_PR_{detection_specification}TPS.png",
                                     supported_models=args.models,
                                     model_names=args.model_names)
    elif args.plot_boxplots_per_type:
        with open(eval_output_path / f"model_2_class_2_metric_vals_{args.eval_output_filename}.pkl", "rb") as file:
            model_2_class_2_ap_vals, model_2_class_2_rocauc_vals, model_2_class_2_mccf1_vals = pickle.load(file)
        for model_2_class_2_metric_vals, metric_name in zip([model_2_class_2_ap_vals, model_2_class_2_rocauc_vals, model_2_class_2_mccf1_vals],
                                                            ['Average Precision', 'ROC AUC', 'MCC-F1 summary']):
            plot_boxplots_per_type(models=args.models, substr_2_type=substrate_2_tps_type,
                                   model_2_class_2_metric_vals=model_2_class_2_metric_vals,
                                   metric_name=metric_name, title='TPS detection per type',
                                   output_path=eval_output_path / f"{plots_name}_{metric_name}_per_type.png",
                                   model_names_plots=args.model_names)
    elif args.plot_barplots_per_category:
        with open(eval_output_path / f"model_2_class_2_metric_vals_{args.eval_output_filename}.pkl", "rb") as file:
            model_2_class_2_ap_vals, model_2_class_2_rocauc_vals, model_2_class_2_mccf1_vals = pickle.load(file)
        for model_2_class_2_metric_vals, metric_name in zip(
                [model_2_class_2_ap_vals, model_2_class_2_rocauc_vals, model_2_class_2_mccf1_vals],
                ['Average Precision', 'ROC AUC', 'MCC-F1 summary']):
            plot_barplots_per_categories(models=args.models,
                                   model_2_class_2_metric_vals=model_2_class_2_metric_vals,
                                         categories_order=args.categories_order,
                                         category_name=args.category_name,
                                   metric_name=metric_name, title=f'TPS detection per {args.category_name.lower()}',
                                   output_path=eval_output_path / f"{plots_name}_{metric_name}_per_{args.category_name.lower()}.png",
                                   model_names_plots=args.model_names)
    else:
        ...
        for metric, column_group in zip(['Mean Average Precision', 'ROC-AUC', 'MCC-F1 summary'],
                                        [['Mean Average Precision (mAP)', 'mAP - SEM', 'mAP + SEM'],
                                         ...
                                         ['MCC-F1 summary (macro mean)', 'Mean MCC-F1 summary - SEM',
            ...
            for model in args.models:
                ...
                means.append(mean_val)
                stds.append(std_val)
            plot_bars(means, stds, args.model_names, "TPS classes detection", metric,

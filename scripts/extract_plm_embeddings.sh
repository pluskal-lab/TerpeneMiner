...
do
  ...
  plm_embeddings --start-index $start_index --end-index $end_index --gpu $gpu_count --model "$model_name" --model-repr-layer "$model_representations_layer" --batch-size $batch_size --csv-path "$csv_path" --id-column "$id_column_name" --seq-column "$sequence_column_name" --output-root-path "$output_root_path" &
  pids+=($!)
...
############ computing number of all samples ############
gather_plm_embeddings --input-root-path "$output_root_path/uniprot_embs_$model_name" --csv-path "$csv_path" --id-column "$id_column_name"